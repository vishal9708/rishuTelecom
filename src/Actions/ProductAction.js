...
export const getProducts =
  (name = "", currentPage = 1, category, ratings = 5) =>
  async (dispatch) => {
    try {
      dispatch({ type: "AllProductsRequest" });

      let link = `${BASE_URL}/api/v1/products/?name=${name}&page=${currentPage}&rating=${ratings}`;

      if (category) {
        link = `${BASE_URL}/api/v1/products/?name=${name}&page=${currentPage}&category=${category}&rating=${ratings}`;
      }
